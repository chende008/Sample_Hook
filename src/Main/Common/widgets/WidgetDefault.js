...
export function RNBtn({ style = {}, key, onPress, ...props }) {
  ...
  let textStyle = (checked || !enabled) ? {
    color: Colors.text_disable,
    backgroundColor: Colors.disable
  } : { color: Colors.text, backgroundColor: Colors.theme };
  return <XText key={key} style={[styles.btnText, { borderRadius: borderRadius }, textStyle, style]} raw={raw}
                text={text} onPress={() => (enabled && onPress) && onPress()} {...props}/>
}
