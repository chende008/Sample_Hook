...
export function NavigationBar({ onBack, clickRText, clickRImage, ...props }) {
  ...
  return <XView style={[styles.titleBarParent, lineStyle]}>
    ...
    {!hideBack &&
    <XImage style={styles.leftImage} icon={ImageRes.img_back} iconSize={22} onPress={() => onBack && onBack()}/>}
    {rightText && <XText style={[styles.rightText]} text={rightText} onPress={() => clickRText && clickRText()}/>}
    {rightImage &&
    <XImage style={styles.rightImage} icon={rightImage} iconSize={22} onPress={() => clickRImage && clickRImage()}/>}
  </XView>;
}
